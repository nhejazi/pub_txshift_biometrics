...
for (estim_type in estim) {
  ...
  sim_results <- map(n_samp, function(sample_size) {
    ...
    result_nsamp <-
      future_lapply(X = seq_len(n_sim), FUN = function(x) {
        ...
        if (param_type == "shift") {
          sim_iter_out <- sim_iter_out %>%
            add_column(sim_id = rep(x, length(shift_grid)))
  ...
  names(sim_results) <- paste("n", n_samp, sep = "_")
  ...
  saveRDS(object = sim_results,
          file = here("data", ifelse(sim_type == "simple", "simple",
                                     paste(sim_type, effect_type,
                                           sep = "_")),
                      paste0("05-ignore_sampling_", estim_type, "_",
                             param_type, "_", timestamp, ".rds"))
         )
}
